...
> **음성(voice_id)은 따로 넣지 않습니다.** FTF 발화 음성은 **LiveAvatar 워크스페이스의 아바타에 이미 설정**돼 있어, 세션 생성 시 `voice_id`를 전달하지 않습니다(참고앱과 동일). `avatar_id`만 주면 그 아바타의 음성으로 말합니다. 선택적으로 지식(RAG) 컨텍스트를 묶으려면 프론트에서 `context_id`를 전달할 수 있습니다.

- **TTT만 쓰려면** `OPENAI_API_KEY` 하나면 됩니다.
- **STS(음성)** 도 `OPENAI_API_KEY` 만 있으면 됩니다 (Realtime 세션이 같은 키로 발급).
- **FTF(아바타)** 는 `LIVEAVATAR_API_KEY` 만 있으면 됩니다 (`AVATAR_ID`는 코드 기본값 내장, voice_id 불필요).

> **이 프로젝트 .env 매핑:** `LIVEAVATAR_API_KEY` = `HEYGEN_LIVE_API_KEY`(값 동일). FTF 아바타는 `index.html` 기본값 `3554efce-af84-4701-981e-2cbd46e991af`(dkpark 워크스페이스, 음성 설정됨)을 사용. 이 아바타·키 조합으로 **voice_id 없이 세션 생성 200 OK 검증 완료**.
...
```js
const AVATAR_ID = window.AVATAR_ID || '3554efce-af84-4701-981e-2cbd46e991af'; // dkpark 워크스페이스(음성 설정됨)
```

기본값은 **dkpark 워크스페이스의 음성 설정된 LiveAvatar 아바타**로 이미 설정돼 있습니다(그래서 `voice_id`가 필요 없습니다). 다른 아바타를 쓰려면:
...
2. 위 표의 환경변수를 등록합니다 (`OPENAI_API_KEY` 필수, FTF 쓰면 `LIVEAVATAR_API_KEY`. AVATAR_ID는 코드 기본값 내장).
...
#   OPENAI_API_KEY=sk-...
#   LIVEAVATAR_API_KEY=...   # voice_id/AVATAR_ID는 불필요 (아바타에 음성 설정·코드 기본값)
vercel dev
```
