...
| `AVATAR_ID` | **FTF** | 사용할 LiveAvatar 아바타 UUID (아래 "AVATAR_ID 설정" 참고) |

- **TTT만 쓰려면** `OPENAI_API_KEY` 하나면 됩니다.
- **STS(음성)** 도 `OPENAI_API_KEY` 만 있으면 됩니다 (Realtime 세션이 같은 키로 발급).
- **FTF(아바타)** 는 `LIVEAVATAR_API_KEY` + `AVATAR_ID` 가 모두 있어야 동작합니다.
...
1. **직접 수정** — `index.html`의 `'REPLACE_WITH_LIVEAVATAR_AVATAR_ID'` 를 실제 LiveAvatar 아바타 UUID로 교체.
