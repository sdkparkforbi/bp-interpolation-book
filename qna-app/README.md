...
| `LIVEAVATAR_CONTEXT_ID` | **FTF (권장)** | LiveAvatar 페르소나 컨텍스트 ID — **음성 + 지식(RAG)** 을 묶음. 이걸 쓰면 voice_id 생략 |
| `LIVEAVATAR_VOICE_ID` | **FTF (대안)** | ElevenLabs 음성 ID. context_id가 없을 때 **필수** |
| `AVATAR_ID` | **FTF** | LiveAvatar 아바타 UUID (프론트 상수, 기본 dkpark) |

> **검증 결과:** LiveAvatar는 **voice_id 또는 context_id 중 하나가 필수**(아바타 기본 음성 없음). `context_id`를 주면 그 컨텍스트가 음성·지식을 제공하므로 가장 깔끔합니다(참고앱 방식). DAY BP 지식은 `../skill/heygen_knowledge.md`를 LiveAvatar 워크스페이스 컨텍스트로 등록 → `LIVEAVATAR_CONTEXT_ID` 사용 권장.
